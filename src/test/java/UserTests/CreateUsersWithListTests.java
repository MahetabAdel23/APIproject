...
public class CreateUsersWithListTests {
    ...
    public void CreatInvalidUserWithNoBody()
    {
        SoftAssert softAssert = new SoftAssert();
        Response response = CreateUsersWithListUtil.CreatUserWithNoBody(405);
        System.out.println("Response JSON Body: " + response.getBody().asString());//print response body
        JsonPath jsonPath = response.jsonPath();
        softAssert.assertEquals(jsonPath.getString("message"),"no data");
