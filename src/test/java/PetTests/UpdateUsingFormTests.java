...
import PetUtils.UpdateUsingFormUtils;
...
public class UpdateUsingFormTests {
    String PetId="";
    @Test(dataProvider = "json_parsing",groups = {"RegressionTesting","SmokeTesting"})
    public void UpdateUsingForm(String data) {
        ...
        Response response2 = UpdateUsingFormUtils.UpdateUsingForm(PetId,200);
    ...
    /*
    @Test
    public void InvalidUpdateUsingForm(String data) {
        SoftAssert softAssert = new SoftAssert();


        Response response2 = UpdateUsingFormUtils.UpdateUsingForm("5",400);
        System.out.println("Response JSON Body: " + response2.getBody().asString());//print response body
        JsonPath jsonPath2 = response2.jsonPath();
        softAssert.assertEquals(jsonPath2.getString("message"), "5");
